...
def _enqueue_ddl_queue_item(target_queue, item):
    ...
    ``_DDL_PENDING_HANDOFF`` tracks ids handed to ``target_queue`` but not yet
    claimed by the worker thread. When that set and ``DDL_QUEUED`` disagree for
    a durable ``Queued`` row, the handoff is stale and safe to replay (#784).
    """
    item_id = None
    if isinstance(item, dict):
        item_id = item.get("id") or item.get("ID")
    if item_id and item_id in comicarr.DDL_QUEUED:
        if item_id in _DDL_PENDING_HANDOFF:
            return False
        row = dl_queries.get_ddl_item(item_id)
        status = str((row or {}).get("status") or "").casefold()
        if status == "downloading":
            return False
        if status == "queued":
            logger.info(
                "[DOWNLOADS] Clearing stale DDL_QUEUED entry for %s and re-enqueueing" % item_id
            )
    ...
    if item_id:
        ...
        _DDL_PENDING_HANDOFF.add(item_id)
    return True
...
def _ddl_downloader_loop(queue, link_type_failure, active_item):
    ...
    while True:
        if comicarr.DDL_LOCK.locked():
            ...
            item = canonical_item
            active_item["value"] = item
            _DDL_PENDING_HANDOFF.discard(item["id"])
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        "[DOWNLOADS-DDL] Unable to persist terminal status for id=%s: %s"
            ...
            if not ddl_finalization["complete"]:
                comicarr.DDL_QUEUED.discard(item["id"])
                comicarr.DDL_STUCK_NOTIFIED.discard(item["id"])
                link_type_failure.pop(item["id"], None)
                ddl_cleanup(item["id"])
...
def ddl_cleanup(item_id):
    """Remove durable DDL metadata once the worker has finished with an id."""
    try:
        db.delete("ddl_info", {"ID": item_id})
    except Exception as e:
        logger.error("[DOWNLOADS-DDL] Unable to delete ddl_info row for id=%s: %s" % (item_id, e))


def _resolve_completed_download_path(nzb_folder, nzb_name):
    """Resolve a completed download path for post-processing.

    Delegates to :func:`comicarr.app.downloads.completed_path.resolve_completed_download_file`.
    """
    return resolve_completed_download_file(nzb_folder, nzb_name)


def _validate_postprocess_item(item):
    """Validate a post-process queue item before execution.

    Delegates to :func:`comicarr.app.downloads.pp_commands.validate_postprocess_item`.
    """
    return validate_postprocess_item(item)


def _configured_roots():
    """Return configured destination roots for post-processing.

    Delegates to :func:`comicarr.app.downloads.pp_commands.configured_roots`.
    """
    return configured_roots()


def postprocess_download(item):
    """Post-process a completed download.

    Validates the queue item, resolves the on-disk artifact, and runs the
    post-processor. Raises :class:`PostProcessCommandError` when validation
    fails so callers can surface a durable failure without silent drops.
    """
    try:
        validate_postprocess_item(item)
    except PostProcessCommandError as e:
        logger.error("[DOWNLOADS-PP] invalid post-process item: %s" % e)
        raise

    nzb_folder = item.get("nzb_folder")
    nzb_name = item.get("nzb_name")
    path = resolve_completed_download_file(nzb_folder, nzb_name)
    if path is None:
        raise PostProcessCommandError("completed download file not found for %s" % nzb_name)

    from comicarr import postprocessor

    pp = postprocessor.PostProcessor(nzb_name, nzb_folder, queue=comicarr.PP_QUEUE)
    pp.Process()
