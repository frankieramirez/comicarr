...
def regenerate_api_key(ctx, username, ip):
    ...
    API_KEY is deliberately absent from WRITABLE_CONFIG_KEYS, so this is the only
    path that may rotate it. Rotation is all-or-nothing: require_api_key() reads
    ctx.config.API_KEY live, so a key that is live but unpersisted would 401 every
    existing client until a restart silently reverted it.
    """
    import comicarr
    ...
    old_api_key = ctx.config.API_KEY
    ...
    try:
        ...
        ctx.config.API_KEY = old_api_key
        logger.error("[CONFIG] API key regeneration failed, reverted to previous key: %s" % e)
